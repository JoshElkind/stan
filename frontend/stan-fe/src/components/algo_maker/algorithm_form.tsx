...
export default function AlgorithmForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // console.log("Const Value:", rowWiseVariables[0].innerVariables[0])
    ...
    for (let i = 0; i < outerConstants.length; i++) {
      ...
      const Variable_type = curr_const.valueType // can be either constant or var...
      const Variable_data = []
            ...
            <div className="space-y-5">
              {rowWiseVariables.map((variable) => (
                ...
                >
                  ...
                  {variable.type === "window" && (
                    <div className="flex items-center gap-3">
                      <Label htmlFor={`name-${variable.id}`} className="text-sm text-slate-300 font-medium w-12">
                        Name:
                      </Label>
                      <Input
                        id={`name-${variable.id}`}
