...
def test1(
    df, pl, gp, lp
):  # dataframe, position length (ticks), gain percentage, loss percentage
    ...
    return [
        ...
        sell_wins,
        sell_losses,
    ]  # returns the gain/loss percentage over all trades, number of positions won, number of positions lost
        ...
        if action != "Hold":
            ...
            while start <= i + pl - 1 and start < length:
                ...
                if action == "Buy" and (start_price - price) / price >= gp:
                    ...
                    action == "Buy"
                    and (start_price - price) < 0
                    and abs((start_price - price) / price) >= lp
                ):
                    ...
                    action == "Sell"
                    and (price - start_price) < 0
                    and abs(price - start_price) / price >= lp
                ):
                    win_loss_setup_ratio -= lp
                    sell_losses += 1
                    # print(4)
                ...
                start += 1
    return [
        win_loss_setup_ratio / (buy_wins + buy_losses + sell_wins + sell_losses),
        buy_wins,
        buy_losses,
        sell_wins,
        sell_losses,
    ]  # percent gain/loss total, number of won positions, and number of lost positions
...
def test3(df, pl, gp, lp):
    ...
    buy_win_upcoming = array(
        *[
            functions.when(
                (lead("price", i).over(w) - col("price")) / col("price") >= gp, lit(1)
            ).otherwise(lit(0))
    ...
    buy_loss_upcoming = array(
        *[
            functions.when(
                (lead("price", i).over(w) - col("price") < 0)
                & (abs((lead("price", i).over(w) - col("price")) / col("price")) >= lp),
                lit(1),
            ).otherwise(lit(0))
            for i in range(1, pl + 1)
        ]
    ...
    sell_win_upcoming = array(
        *[
            functions.when(
                (col("price") - lead("price", i).over(w)) / col("price") >= gp, lit(1)
            ).otherwise(lit(0))
            for i in range(1, pl + 1)
        ]
    )
    sell_loss_upcoming = array(
        *[
            functions.when(
                (col("price") - lead("price", i).over(w) < 0)
                & (abs((col("price") - lead("price", i).over(w)) / col("price")) >= lp),
                lit(1),
            ).otherwise(lit(0))
            for i in range(1, pl + 1)
        ]
    ...
    df_win_loss_buy = df.withColumn(
        ...
        functions.when(
            ...
            lit(
                array(
                    lit(functions.array_position(buy_win_upcoming, 1)),
                    lit(functions.array_position(buy_loss_upcoming, 1)),
                )
            ),
        ).otherwise(lit(array(lit(0), lit(0)))),
    )
    ...
    df_win_loss_sell = df.withColumn(
        ...
        functions.when(
            ...
            lit(
                array(
                    lit(functions.array_position(sell_win_upcoming, 1)),
                    lit(functions.array_position(sell_loss_upcoming, 1)),
                )
            ),
        ).otherwise(lit(array(lit(0), lit(0)))),
    )
    ...
    rows = df_win_loss_buy.filter(col("win-loss-arr") == 2).select("count").collect()
    win_buy = rows[0][0] if rows else 0

    rows = df_win_loss_buy.filter(col("win-loss-arr") == 1).select("count").collect()
    loss_buy = rows[0][0] if rows else 0

    # total_buy
    # total_buy
    rows = df_win_loss_buy.filter(col("win-loss-arr") == 0).select("count").collect()
    total_buy_base = rows[0][0] if rows else 0
    total_buy = total_buy_base + win_buy + loss_buy

    # win_sell
    rows = df_win_loss_sell.filter(col("win-loss-arr") == 2).select("count").collect()
    win_sell = rows[0][0] if rows else 0

    # loss_sell
    rows = df_win_loss_sell.filter(col("win-loss-arr") == 1).select("count").collect()
    loss_sell = rows[0][0] if rows else 0
   
    rows = df_win_loss_sell.filter(col("win-loss-arr") == 0).select("count").collect()
    total_sell_base = rows[0][0] if rows else 0
    total_sell = total_sell_base + win_sell + loss_sell

    arr_final = [win_buy, loss_buy, win_sell, loss_sell, total_buy, total_sell]

    if (win_buy + win_sell + loss_buy + loss_sell) == 0:
        qual = 0
    else:
        qual = ((win_buy + win_sell) * (gp / lp) - (loss_buy + loss_sell) * (lp)) / (win_buy + win_sell + loss_buy + loss_sell)
