...
from pyspark.sql.functions import lag, lead, avg, max, coalesce, lit, sum, array, col, abs, when, monotonically_increasing_id
...
def algos_combine_arr_format(post_algos, intercept_range, intercept_needed, num_rows): 
    ...
    verdict = sliding_verdict(actions, pos_idx, pos_flat, intercept_range, intercept_needed)
...
def algos_combine_arr_format_slow(post_algos, intercept_range, intercept_needed, num_rows): 
    # intercept range is the ammount of tick spand that we need to find intercept_needed overlaps with the algos
    actions_track = []
    default_df = post_algos[0]
    print("Combining algos for single asset.")
...
def asset_test_single(arr_asset, postition_length, gain_percentage, loss_percentage, algos, intercept_range, clean_range, intercept_needed):
    spark = SparkSession.builder.appName("converter").config("spark.driver.memory", "12g").config("spark.executor.memory", "12g").config("spark.driver.cores", "6").config("spark.executor.cores", "6").config("spark.sql.shuffle.partitions", "200").getOrCreate()
    ...
    data_single_asset = data_single_asset.withColumn("row_id", monotonically_increasing_id().cast(IntegerType()))
    action_df = spark.createDataFrame([(i, combined_algos[i]) for i in range(len(combined_algos))], ["row_id", "Action"])
    data_single_asset = data_single_asset.join(action_df, on="row_id", how="inner")
    data_single_asset = data_single_asset.withColumn("price", (col("open") + col("close")) / 2)
...
test1 = call_assets(['S'], 40, 0.02, 0.02, [vwap_pullback_strategy], 40, 3, 1)
print(test1[0]) 